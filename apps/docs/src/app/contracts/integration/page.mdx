...
async function createPublication(params: CreatePublicationParams) {
  ...
  txb.transferObjects(
    [publication, publisherCap, stats],
  ...
  if (params.statsId) {
    ...
    if (params.tier !== Tier.Free) {
      txb.moveCall({
        ...
        arguments: [
          txb.object(params.statsId),
          txb.pure.u64(params.paymentAmount),
        ],
      });
    }
  }
...
async function deleteComment(commentId: string) {
  ...
  txb.moveCall({
    ...
    arguments: [txb.object(commentId), txb.object(CLOCK_ID)],
  });
...
async function subscribeWithTreasuryFee(params: SubscribeWithFeeParams) {
  ...
  // Split coins for payment
  let [paymentCoin] = txb.splitCoins(txb.gas, [
    txb.pure.u64(params.paymentAmount),
  ]);
...
const depositRequired = await calculateArticleDeposit(
  TREASURY_ID,
  premiumPrice
);
...
- [Module API](/contracts/api) - Complete function reference
- [Data Structures](/contracts/data-structures) - Struct documentation
- [Events](/contracts/events) - Event schemas
- [Security](/contracts/security) - Security best practices
