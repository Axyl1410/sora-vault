...
```typescript
const { publications, create, update, publish, isLoading } = usePublication();
```

**useSubscription**

```typescript
const { subscribe, renew, listForSale, mySubscriptions, checkAccess } =
  useSubscription();
```
...
```typescript
const { article, decrypt, isDecrypting, error } =
...
```typescript
const { upload, download, isUploading } = useWalrus();
```
...
- Seal encryption happens _before_ Article object creation, using a random hex ID
