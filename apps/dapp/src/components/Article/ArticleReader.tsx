...
import { type Article, Tier } from "../../types";
import { logger } from "../../utils/logger";
import { saveReadingProgress } from "../../utils/readingProgress";
import { ArticleEpochExtension } from "./ArticleEpochExtension";
...
export function ArticleReader({
	...
	const { readArticle, isReading, error, sessionReady } =
		useReadEncryptedArticle();
	const { initializeSession, isInitializing } = useSealSession();

	const hasAccess = !!subscriptionId || !!readTokenId || !!publisherCapId;
	const accessMethod = subscriptionId
		? "subscription"
		: readTokenId
			? "token"
			: publisherCapId
	...
	const handleDecrypt = async () => {
		...
		// Validate owner access has required parameters
		if (accessMethod === "owner" && (!publisherCapId || !publicationId)) {
			logger.error(
				{
					context: "ArticleReader",
					operation: "decrypt",
				...
				"Owner access requires PublisherCap ID and Publication ID",
			);
			return;
		}
	...
	const calculateDailyPrice = (tier: Tier): bigint => {
		...
		const monthlyPrice =
			tier === Tier.Basic ? PRICING.BASIC_MONTHLY : PRICING.PREMIUM_MONTHLY;
				...
				<Flex direction="column" gap="4">
					...
					<Box>
						...
						<Flex gap="2" mt="3">
							<Text size="1" color="gray">
								Published:{" "}
								{new Date(
									Number(article.published_at) * 1000,
								).toLocaleDateString()}
							</Text>
							{article.is_archived && (
					...
					{!hasAccess && sessionReady && (
						<Callout.Root color="amber">
							...
							<Callout.Text>
								<Box mb="2">
									<Text weight="bold">This article is encrypted</Text>
								</Box>
								<Box mb="3">
									<Text size="2" color="gray">
										Subscribe or purchase a 24-hour read token to access this
										content.
									</Text>
								</Box>
								<Flex gap="2">
									...
									{onPurchaseToken && article.tier !== Tier.Free && (
										<Button size="2" variant="soft" onClick={onPurchaseToken}>
											Buy Token (
											{(
												calculateDailyPrice(article.tier) / 1_000_000_000n
											).toString()}{" "}
											SUI for 24h)
										</Button>
									)}
						...
						<>
							{!decryptedContent && !isDecrypting && (
								<Callout.Root color="green">
									...
									<Callout.Text>
										<Box mb="2">
											<Text weight="bold">Access granted</Text>
										</Box>
										<Box mb="3">
											<Text size="2" color="gray">
												You have{" "}
							...
							{(isDecrypting || isReading) && (
								<Callout.Root>
									...
									<Callout.Text>
										<Box mb="1">
											<Text weight="bold">Decrypting article...</Text>
										</Box>
										<Text size="1" color="gray">
					...
					{error && (
						<Callout.Root color="red">
							...
							<Callout.Text>
								<Box mb="1">
									<Text weight="bold">Decryption failed</Text>
								</Box>
					...
					<Card variant="surface">
						<Flex direction="column" gap="2">
							...
							<Text size="1" color="gray">
								📦 Stored on Walrus (Blob ID:{" "}
								{article.walrus_blob_id.slice(0, 16)}...)
							</Text>
					...
					{/* Epoch Extension Section */}
					<ArticleEpochExtension article={article} />
				</Flex>
			</Card>
		</Container>
	);
