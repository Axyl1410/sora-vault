...
import { graphql, suiGraphQLClient } from "../config/graphql";
import type {
	Article,
	Publication,
	PublicationStats,
	...
	for (const node of result.data?.objects.nodes || []) {
		if (node?.asMoveObject?.contents?.json) {
			...
			if (data.publication_id === publicationId) {
				...
				const sealKeyIdBytes =
					typeof data.seal_key_id === "string"
...
export async function fetchUserSubscriptions(
    ...
    query GetUserSubscriptions($typeFilter: String!, $owner: SuiAddress!) {
      objects(filter: { type: $typeFilter, owner: $owner }) {
        nodes {
          address
          asMoveObject {
            contents {
 ...
 * Returns the access method (subscription, token, or owner) if user has access
 */
export async function checkUserAccess(
	...
	hasAccess: boolean;
	method?: "subscription" | "token" | "owner";
	objectId?: string;
}> {
	// First, check if user is the creator of the publication
	const publication = await fetchPublicationById(article.publication_id);
	if (
		publication &&
		publication.creator.toLowerCase() === userAddress.toLowerCase()
	) {
		return {
			hasAccess: true,
			method: "owner",
