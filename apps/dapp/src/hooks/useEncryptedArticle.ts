...
import {
	decryptWithOwner,
	decryptWithReadToken,
	decryptWithSubscription,
	encryptArticleContent,
} from "../services/seal";
import { fetchFromWalrus, uploadToWalrus } from "../services/walrus";
...
export interface ReadArticleParams {
	...
	accessMethod: "subscription" | "token" | "owner";
...
export function usePublishEncryptedArticle() {
	...
	const publishArticle = async (params: PublishArticleParams) => {
		...
		try {
			...
			const { encryptedData, metadata: sealMetadata } =
				await encryptArticleContent(
					params.content,
			...
			const error =
				err instanceof Error ? err : new Error("Failed to publish article");
			setError(error);
			throw error;
		} finally {
		...
		try {
			...
			logDebug(
				{
					...
					blobId: params.article.walrus_blob_id,
				},
			...
			if (params.accessMethod === "subscription") {
				if (!params.subscriptionId) {
					throw new Error(
						"Subscription ID required for subscription-based access",
					);
				}
			...
			} else if (params.accessMethod === "owner") {
				if (!params.publisherCapId || !params.publicationId) {
					throw new Error(
						"PublisherCap ID and Publication ID required for owner-based access",
					);
				}

				decryptedContent = await decryptWithOwner({
					encryptedData,
					sessionKey,
					suiClient,
					packageId,
					moduleName: "seal_policy",
					publisherCapId: params.publisherCapId,
					articleId: params.article.id,
					publicationId: params.publicationId,
					sealKeyId: params.article.seal_key_id,
				});
			...
			const error =
				err instanceof Error ? err : new Error("Failed to read article");
			setError(error);
			throw error;
		} finally {
			setIsReading(false);
...
export function useGenerateReadToken() {
	...
	const generateToken = async (
		...
		try {
			...
			const error =
				err instanceof Error ? err : new Error("Failed to generate read token");
			setError(error);
			throw error;
		} finally {
