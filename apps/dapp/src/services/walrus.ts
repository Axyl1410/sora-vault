...
export async function uploadToWalrus(
	...
	try {
		...
		logDebug(
			{ context: "walrus", operation: "upload", step: 1 },
			"Step 1/4: Encoding files",
		);
		await flow.encode();

		// Step 2: Register on-chain (requires signing)
		logDebug(
			{ context: "walrus", operation: "upload", step: 2 },
		...
		logDebug(
			{ context: "walrus", operation: "upload", txDigest: digest },
			`Registration digest: ${digest}`,
		);

		// Step 3: Upload to storage nodes (requires digest from registration)
		logDebug(
			{ context: "walrus", operation: "upload", step: 3, txDigest: digest },
			"Step 3/4: Uploading to storage nodes",
		);
		await flow.upload({ digest });

		// Step 4: Certify on-chain (requires signing)
		logDebug(
			{ context: "walrus", operation: "upload", step: 4 },
			"Step 4/4: Certifying on-chain",
		);
...
export async function fetchFromWalrus(
	...
	try {
		logInfo(
			...
			`Fetching and parsing quilt blob: ${blobId}`,
		);
...
/**
 * Upload an image file to Walrus and return blob ID with dimensions
 *
 * @param file - Image file to upload
 * @param suiClient - Sui client for certificate transactions
 * @param ownerAddress - Address that will own the blob
 * @param signAndExecute - Function to sign and execute transactions
 * @param epochs - Storage duration in epochs (default: 5)
 * @returns Blob ID, width, and height
 */
export async function uploadImageToWalrus(
	...
	signAndExecute: (params: { transaction: Transaction }) => Promise<any>,
	epochs: number = 5,
): Promise<{ blobId: string; width: number; height: number }> {
	try {
		logInfo(
			{
				context: "walrus",
			...
			"Starting image upload to Walrus",
		);

		// 1. Read file as ArrayBuffer and convert to Uint8Array
		...
		// 3. Upload to Walrus using existing function
		...
		logInfo(
			...
			`Image uploaded successfully: ${width}x${height}`,
		);
		...
		logError(
			{ context: "walrus", operation: "uploadImage", fileName: file.name },
			"Image upload failed",
			error instanceof Error ? error : new Error(String(error)),
		);
		throw new Error(
...
/**
 * Get image dimensions from a file using HTML Image element
 *
...
/**
 * Extend the storage duration of a blob on Walrus
 *
 * @param blobId - Blob ID to extend
 * @param additionalEpochs - Number of additional epochs to add
 * @param suiClient - Sui client for transactions
 * @param ownerAddress - Address that will pay for the extension
 * @param signAndExecute - Function to sign and execute transactions
 * @returns Transaction digest
 */
export async function extendBlobEpochs(
	blobId: string,
	...
	try {
		logInfo(
			{ context: "walrus", operation: "extendBlob", blobId, additionalEpochs },
			`Extending blob ${blobId} by ${additionalEpochs} epochs`,
		);
		...
		// The extend method should create a transaction to extend storage
		const tx = walrus.extend({
			blobId,
		...
		logInfo(
			{
				context: "walrus",
				operation: "extendBlob",
				blobId,
				txDigest: result.digest,
			},
			`Blob extension successful - digest: ${result.digest}`,
		);
		...
		logError(
			{ context: "walrus", operation: "extendBlob", blobId, additionalEpochs },
			"Blob extension failed",
			error instanceof Error ? error : new Error(String(error)),
		);
		throw new Error(
			`Failed to extend blob: ${error instanceof Error ? error.message : "Unknown error"}`,
		);
	}
}
