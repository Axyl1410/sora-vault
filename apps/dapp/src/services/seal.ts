...
import { EncryptedObject, SealClient, type SessionKey } from "@mysten/seal";
...
export interface DecryptWithOwnerParams {
	encryptedData: Uint8Array;
	sessionKey: SessionKey;
	suiClient: SuiClient;
	packageId: string;
		...
		logDebug(
			{
				...
				serverCount: keyServerObjectIds.length,
			},
...
export async function encryptArticleContent(
	...
	if (keyServerObjectIds.length === 0) {
		throw new Error(
			"No key servers configured. Please configure Seal key servers in networkConfig.",
		);
	}

	if (threshold > keyServerObjectIds.length) {
		throw new Error(
			`Threshold (${threshold}) cannot exceed number of key servers (${keyServerObjectIds.length})`,
		);
	}
	...
	logDebug(
		...
		`Encrypted object first 100 bytes (hex): ${Array.from(
			encryptedObject.slice(0, 100),
		)
		...
		logError(
			{
				...
				operation: "encrypt",
				articleId,
				inputServerCount: keyServerObjectIds.length,
			},
		...
		logInfo(
			{
				...
				operation: "encrypt",
				articleId,
				serviceCount: parsedEncryptedObject.services.length,
			},
...
export async function decryptWithSubscription(
	...
	logDebug(
		...
		`Encrypted data first 100 bytes (hex): ${Array.from(
	...
	logDebug(
		{
			...
			articleId,
			subscriptionId,
			serverCount: keyServerIds.length,
		},
...
/**
 * Decrypt article content using owner/publisher access
 *
 * Builds a transaction that calls seal_approve_owner to verify access.
 * Key servers will execute this transaction to validate the user's ownership.
 *
 * @param params - Decryption parameters
 * @returns Decrypted article content
 */
export async function decryptWithOwner(
	params: DecryptWithOwnerParams,
): Promise<string> {
	const {
		encryptedData,
		sessionKey,
	...
	logInfo(
		...
		`Decrypting with owner access (publisher cap ${publisherCapId})`,
	);

	logDebug(
		{ context: "seal", operation: "decrypt", articleId, publisherCapId },
		`Encrypted data first 100 bytes (hex): ${Array.from(
			encryptedData.slice(0, 100),
		)
			.map((b) => b.toString(16).padStart(2, "0"))
	...
	// Build transaction that calls seal_approve_owner
	const tx = new Transaction();
	tx.moveCall({
		target: `${packageId}::${moduleName}::seal_approve_owner`,
		arguments: [
			tx.pure.vector("u8", fromHex(encryptedObject.id)), // Article ID
			tx.object(publisherCapId), // PublisherCap
			tx.object(articleId), // Article object
	...
	logInfo(
		...
		"Decryption with owner access successful",
	);
...
export function clearSealClientCache(): void {
	...
	logDebug(
		{ context: "seal", operation: "clearCache" },
