...
describe("Article Module", () => {
	...
	it("should publish an article successfully", async () => {
		...
		const capObject = pubResult.objectChanges?.find(
			(change, idx) =>
				idx > 0 && change.type === "created" && "objectId" in change,
		);
		...
		const [deposit] = articleTx.splitCoins(articleTx.gas, [
			articleTx.pure.u64(depositAmount),
		]);
		...
		articleTx.moveCall({
			...
			arguments: [
				...
				articleTx.pure.vector(
					"u8",
					Array.from(Buffer.from(TEST_ARTICLE.sealKeyId)),
				),
				basicTier,
	...
	it("should allow multiple users to extend the same article", async () => {
		...
		if (event1?.parsedJson) {
			expect((event1.parsedJson as any).extender).toBe(
				extender1.toSuiAddress(),
			);
		...
		if (event2?.parsedJson) {
			expect((event2.parsedJson as any).extender).toBe(
				extender2.toSuiAddress(),
			);
		...
		if (event3?.parsedJson) {
			expect((event3.parsedJson as any).extender).toBe(
				extender3.toSuiAddress(),
			);
