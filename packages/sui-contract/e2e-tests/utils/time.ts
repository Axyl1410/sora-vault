 ...
 */
export async function calculateSubscriptionExpiry(
	client: SuiClient,
): Promise<number> {
 ...
 */
export async function calculateReadTokenExpiry(
	client: SuiClient,
): Promise<number> {
	const now = await getCurrentTimestamp(client);
 ...
 */
export async function getPastTimestamp(
	client: SuiClient,
	secondsAgo: number,
): Promise<number> {
	const now = await getCurrentTimestamp(client);
 ...
 */
export async function getFutureTimestamp(
	client: SuiClient,
	secondsAhead: number,
): Promise<number> {
	const now = await getCurrentTimestamp(client);
