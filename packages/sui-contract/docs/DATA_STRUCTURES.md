...
| `image_metadata` | `String` | JSON metadata for images (blob IDs, dimensions, alt text, etc.) |
...
- `image_metadata` - Can update via `update_article()`
- `is_archived` - Can toggle via `archive_article()`

**Image Metadata Format:**

The `image_metadata` field stores JSON-formatted metadata for article images:

```json
{
  "images": [
    {
      "blob_id": "walrus_blob_id_1",
      "width": 1920,
      "height": 1080,
...
**Image Storage:**
- Images are uploaded to Walrus as separate blobs
- Each image blob ID is stored in the metadata
- Images can be encrypted with Seal (same key as article content)
- Frontend handles image upload/retrieval separately from article content
...
On-chain comment with nested reply support.
...
| Field | Type | Description |
|-------|------|-------------|
| `id` | `UID` | Unique comment identifier |
| `article_id` | `ID` | Article this comment belongs to |
| `author` | `address` | Comment author's address |
| `parent_comment_id` | `Option<ID>` | Parent comment ID for nested replies (None for top-level) |
| `content` | `String` | Comment text content |
| `created_at` | `u64` | Creation timestamp (seconds) |
| `updated_at` | `u64` | Last update timestamp (seconds) |
| `is_deleted` | `bool` | Soft delete flag (true if deleted) |
...
```
Article
  ├─ Comment A (parent_comment_id: None)
  │   ├─ Reply A1 (parent_comment_id: Comment A)
  │   └─ Reply A2 (parent_comment_id: Comment A)
  │       └─ Reply A2a (parent_comment_id: Reply A2)
  └─ Comment B (parent_comment_id: None)
      └─ Reply B1 (parent_comment_id: Comment B)
```

**Access Control:**

Comments require:
- Valid subscription to article's publication
...
- Content is preserved (for audit trail)
- Comment is hidden from UI
- Replies to deleted comments are prevented

**Storage Model:**

Comments are stored as shared objects to enable:
- Efficient querying by article_id
- Off-chain indexing and aggregation
- Real-time comment feeds
- Thread building from events
...
| `article_id` | 32 bytes |
| `author` | 32 bytes |
| `parent_comment_id` | 33 bytes (Option) |
| `content` | Variable (typically 50-500 bytes) |
| `created_at` | 8 bytes |
| `updated_at` | 8 bytes |
| `is_deleted` | 1 byte |
| **Total** | ~200-700 bytes per comment |
