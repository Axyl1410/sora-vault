...
```typescript
interface ChangeTierParams {
  oldSubscriptionId: string;
  publicationId: string;
  treasuryId: string;
  newTier: Tier;
  paymentAmount: bigint; // May be 0 if downgrading
}

async function changeTier(params: ChangeTierParams) {
  const txb = new TransactionBlock();

  // Calculate required payment (optional - can be done off-chain)
  // If upgrading, payment = new_price - remaining_value
  // If downgrading, payment = 0

  const [coin] = txb.splitCoins(txb.gas, [txb.pure.u64(params.paymentAmount)]);

  const [newSubscription] = txb.moveCall({
    target: `${PACKAGE_ID}::subscription::change_tier`,
    arguments: [
      txb.object(params.oldSubscriptionId),
      txb.object(params.publicationId),
      txb.object(params.treasuryId),
      txb.pure.u8(params.newTier),
      coin,
      txb.object(CLOCK_ID),
    ],
  });
  ...
  const sender = await client.getActiveAddress();
  txb.transferObjects([newSubscription], txb.pure.address(sender));

  return executeTransaction(txb);
}

// Usage - Upgrade from Basic to Premium
await changeTier({
  oldSubscriptionId: '0x...',
  publicationId: '0x...',
  treasuryId: '0x...',
  newTier: Tier.Premium,
  paymentAmount: 12_500_000_000n, // Calculated: 15 SUI - 2.5 SUI remaining
});
```

### Change Tier with Auto-List on Kiosk

```typescript
interface ChangeTierWithKioskParams {
  oldSubscriptionId: string;
  publicationId: string;
  treasuryId: string;
  newTier: Tier;
...
async function changeTierWithKiosk(params: ChangeTierWithKioskParams) {
  const txb = new TransactionBlock();

  // Calculate suggested price (90% of remaining value)
  // Can use calculate_suggested_price helper function

  const [coin] = txb.splitCoins(txb.gas, [txb.pure.u64(params.paymentAmount)]);

  const [newSubscription] = txb.moveCall({
    target: `${PACKAGE_ID}::subscription::change_tier_with_kiosk`,
    arguments: [
      txb.object(params.oldSubscriptionId),
      txb.object(params.publicationId),
      txb.object(params.treasuryId),
      txb.pure.u8(params.newTier),
      coin,
      txb.object(params.kioskId),
      txb.object(params.kioskCapId),
      txb.pure.u64(params.suggestedPrice),
      txb.object(CLOCK_ID),
    ],
  });
...
async function calculateRemainingValue(
  ...
  const result = txb.moveCall({
    target: `${PACKAGE_ID}::subscription::calculate_remaining_value`,
    arguments: [
  ...
  // Parse u64 result
  const value = simulation.results?.[0]?.returnValues?.[0];
...
async function updateListingPrice(
  ...
  txb.moveCall({
    target: `${PACKAGE_ID}::subscription::update_listing_price`,
    arguments: [
      txb.object(kioskId),
      txb.object(kioskCapId),
      txb.object(subscriptionId),
      txb.pure.u64(newPrice),
    ],
  });
...
### Publish Article (with Encryption and Images)
...
interface PublishArticleParams {
  ...
  tier: Tier;
  images?: ImageMetadata[]; // Optional images
}
...
async function publishArticle(params: PublishArticleParams) {
  ...
  // 3. Upload images to Walrus (if provided)
  const imageMetadata: ImageMetadata[] = [];
  if (params.images) {
    for (const image of params.images) {
      // Upload image blob
      const imageBlobId = await walrusClient.upload(image.file);
      imageMetadata.push({
        blob_id: imageBlobId,
        width: image.width,
        height: image.height,
  ...
  // 4. Calculate deposit (1% of premium price)
  const depositAmount = await calculateArticleDeposit(
    params.treasuryId,
    params.publicationId
  );

  // 5. Create article on-chain
  const txb = new TransactionBlock();
  ...
  txb.moveCall({
    ...
    arguments: [
      ...
      txb.pure.string(JSON.stringify({ images: imageMetadata })),
      depositCoin,
    ],
  });
...
async function updateArticle(params: UpdateArticleParams) {
  const txb = new TransactionBlock();

  // Prepare image metadata JSON
  const imageMetadataJson = params.imageMetadata
    ? JSON.stringify({ images: params.imageMetadata })
    : '{}';

  txb.moveCall({
    target: `${PACKAGE_ID}::article::update_article`,
    arguments: [
      txb.object(params.articleId),
      txb.object(params.publisherCapId),
      txb.pure.string(params.title),
      txb.pure.string(params.excerpt),
      txb.pure.string(imageMetadataJson),
    ],
  });
...
async function createComment(params: CreateCommentParams) {
  ...
  const [comment] = txb.moveCall({
    target: `${PACKAGE_ID}::comment::create_comment`,
    arguments: [
      txb.object(params.articleId),
      txb.object(params.subscriptionId),
      txb.pure.string(params.content),
      txb.object(CLOCK_ID),
    ],
  });

  const sender = await client.getActiveAddress();
  txb.transferObjects([comment], txb.pure.address(sender));
...
async function replyToComment(params: ReplyToCommentParams) {
  const txb = new TransactionBlock();

  const [reply] = txb.moveCall({
    target: `${PACKAGE_ID}::comment::reply_to_comment`,
    arguments: [
      txb.object(params.articleId),
      txb.object(params.subscriptionId),
      txb.object(params.parentCommentId),
      txb.pure.string(params.content),
      txb.object(CLOCK_ID),
    ],
  });
  ...
  const sender = await client.getActiveAddress();
  txb.transferObjects([reply], txb.pure.address(sender));
...
async function deleteComment(commentId: string) {
  ...
  txb.moveCall({
    target: `${PACKAGE_ID}::comment::delete_comment`,
    arguments: [
      txb.object(commentId),
      txb.object(CLOCK_ID),
    ],
...
async function getArticleComments(articleId: string): Promise<Comment[]> {
  ...
  // Filter by article and fetch full comment objects
  const commentIds = events.data
    .filter((event: any) => event.parsedJson.article_id === articleId)
    .map((event: any) => event.parsedJson.comment_id);

  // Fetch comment objects
  const comments = await client.multiGetObjects({
    ids: commentIds,
    options: { showContent: true },
  });
  ...
  return comments
    ...
    .map((obj) => {
      ...
      return {
        ...
        parentCommentId: fields.parent_comment_id || null,
...
function buildCommentThread(comments: Comment[]): Comment[] {
