...
├── article.move            # Encrypted article publishing (with image support)
├── subscription.move       # NFT-based subscriptions (with tier changes)
├── comment.move            # On-chain comment system
...
Manages encrypted articles stored on Walrus with image support.

**Key Structs:**
- `Article` - Encrypted article with Walrus blob reference and image metadata

**Key Functions:**
- `publish_article()` - Publish encrypted article (with image metadata)
- `update_article()` - Update article metadata (title, excerpt, images)
- `archive_article()` - Hide article from listings

**Features:**
- Image metadata stored as JSON (blob IDs, dimensions, alt text)
- Images uploaded separately to Walrus
- Images can be encrypted with Seal (same key as content)

[→ Full API Reference](./MODULE_API.md#article)

### Module: `subscription`

NFT-based subscriptions with tier-based access and marketplace integration.
...
- `change_tier()` - Upgrade/downgrade subscription with prorated payment
- `change_tier_with_kiosk()` - Change tier and auto-list old subscription
- `update_listing_price()` - Update Kiosk listing price
- `calculate_remaining_value()` - Calculate prorated subscription value
- `calculate_suggested_price()` - Suggest marketplace price
...
On-chain comment system with nested replies and subscription-based access.

**Key Structs:**
- `Comment` - Comment with parent reference for threading

**Key Functions:**
- `create_comment()` - Create comment on article
- `reply_to_comment()` - Create nested reply
- `update_comment()` - Update comment content (author only)
- `delete_comment()` - Soft delete comment (author only)

**Features:**
- Subscription-based access control
- Nested reply support
- Soft delete (preserves content for audit)
- Shared objects for efficient querying

[→ Full API Reference](./MODULE_API.md#comment)
...
### comment.move
- `EInsufficientAccess = 1` - Subscription doesn't have required tier access
- `EUnauthorized = 2` - User is not the comment author
- `ECommentDeleted = 3` - Comment is already deleted
- `EInvalidParentComment = 4` - Parent comment is deleted or invalid
...
Change Tier (Upgrade/Downgrade)
   ↓
Auto-list old subscription (optional)
```
