...
public struct Article has key, store {
    ...
    published_at: u64,
    is_archived: bool,
    image_metadata: String,  // JSON metadata for images (blob IDs, dimensions, etc)
}
```
...
- `treasury` - Shared treasury object (for deposit collection)
- `publisher_cap` - Ownership proof
- `title` - Article title
...
- `image_metadata` - JSON string containing image metadata (blob IDs, dimensions, etc.)
- `deposit` - Non-refundable deposit coin (1% of premium price)
- `ctx` - Transaction context

**Returns:** None (article is shared automatically)

**Note:** Requires a deposit equal to 1% of the publication's premium tier price.
...
Updates article metadata (title, excerpt, and image metadata).
...
- `title` - New title
- `excerpt` - New excerpt
- `image_metadata` - New image metadata JSON string
...
public fun image_metadata(article: &Article): String
```
...
- `treasury` - Shared treasury object (for fee collection)
...
Upgrades or downgrades a subscription to a new tier with prorated payment/refund.

**Parameters:**
- `old_subscription` - Current subscription NFT (will be destroyed)
- `publication` - Associated publication
- `treasury` - Shared treasury object
- `new_tier` - Target tier (Free/Basic/Premium)
- `payment` - Payment coin (may be zero if downgrading)
- `clock` - Shared clock object
- `ctx` - Transaction context

**Returns:** New `SubscriptionNFT` with updated tier

**Payment Logic:**
- Calculates remaining value of old subscription (prorated based on time left)
- If upgrading: User pays difference (new_price - remaining_value)
- If downgrading: User pays 0 (remaining value covers new tier)
- New subscription gets remaining time + 30 days
...
**Events:**
- Emits `SubscriptionTierChanged`

**Example:**
```typescript
// Upgrade from Basic (5 SUI) to Premium (15 SUI)
// Subscription has 15 days remaining (50% of month)
// Remaining value = 5 SUI * 0.5 = 2.5 SUI
// Required payment = 15 SUI - 2.5 SUI = 12.5 SUI

const [coin] = txb.splitCoins(txb.gas, [12_500_000_000n]);

const [newSubscription] = txb.moveCall({
  target: `${packageId}::subscription::change_tier`,
  arguments: [
    txb.object(oldSubscriptionId),
    txb.object(publicationId),
...
**Parameters:**
- `old_subscription` - Current subscription NFT
- `publication` - Associated publication
- `treasury` - Shared treasury object
- `new_tier` - Target tier
- `payment` - Payment coin for new tier
- `kiosk` - User's Kiosk object
- `cap` - Kiosk owner capability
- `suggested_price` - Price to list old subscription at
- `clock` - Shared clock object
- `ctx` - Transaction context
...
**Behavior:**
- If old subscription has remaining value: Places it in Kiosk and lists for sale
- If no remaining value: Uses regular `change_tier()` logic
- New subscription gets full 30 days (doesn't carry over remaining time)

**Use Case:** User wants to upgrade and sell their old subscription to recoup costs.

**Example:**
```typescript
// Calculate suggested price (90% of remaining value)
const suggestedPrice = await calculateSuggestedPrice(oldSubscription);

const [coin] = txb.splitCoins(txb.gas, [15_000_000_000n]);

const [newSubscription] = txb.moveCall({
  target: `${packageId}::subscription::change_tier_with_kiosk`,
  arguments: [
    txb.object(oldSubscriptionId),
    txb.object(publicationId),
    txb.object(treasuryId),
...
// Calculate suggested marketplace price
...
Calculates the prorated value of a subscription based on remaining time.

**Formula:**
```
remaining_value = (tier_price * remaining_seconds) / SECONDS_PER_MONTH
```

**Returns:** Value in MIST (0 if expired or free tier)

**`calculate_suggested_price`**

Suggests a marketplace price for selling a subscription (90% of remaining value).

**Returns:** Suggested price in MIST

**Example:**
```typescript
// Calculate remaining value
const remainingValue = await client.devInspectTransactionBlock({
  transactionBlock: txb.moveCall({
    target: `${packageId}::subscription::calculate_remaining_value`,
    arguments: [
...
// Get suggested price (10% discount)
const suggestedPrice = await client.devInspectTransactionBlock({
  transactionBlock: txb.moveCall({
    target: `${packageId}::subscription::calculate_suggested_price`,
    arguments: [
      txb.object(subscriptionId),
      txb.object(publicationId),
      txb.object('0x6'),
...
**Source:** `sources/comment.move`

On-chain comment system with nested reply support and subscription-based access control.
...
- `subscription` - User's subscription NFT (must have access to article tier)
...
**Aborts:**
- `EInsufficientAccess` if subscription doesn't have required tier access
- `EInvalidSubscription` if subscription doesn't match article's publication

**Access Control:**
- Verifies subscription belongs to article's publication
- Verifies subscription has sufficient tier access to article
- Verifies subscription is not expired
...
**Example:**
```typescript
const [comment] = txb.moveCall({
  target: `${packageId}::comment::create_comment`,
  arguments: [
    txb.object(articleId),
    txb.object(subscriptionId),
    txb.pure.string("Great article!"),
    txb.object('0x6'),
  ],
});

txb.transferObjects([comment], sender);
```
...
**Parameters:**
- `article` - Article the comment thread belongs to
...
**Returns:** `Comment` object with `parent_comment_id` set

**Aborts:**
- `EInsufficientAccess` if subscription doesn't have access
- `EInvalidParentComment` if parent comment is deleted

**Events:**
- Emits `CommentCreated` with `parent_comment_id` set

**Example:**
```typescript
const [reply] = txb.moveCall({
  target: `${packageId}::comment::reply_to_comment`,
  arguments: [
    txb.object(articleId),
    txb.object(subscriptionId),
    txb.object(parentCommentId),
    txb.pure.string("I agree!"),
    txb.object('0x6'),
  ],
});
```
...
Updates the content of a comment (author only).
...
**Aborts:**
- `EUnauthorized` if sender is not the comment author
- `ECommentDeleted` if comment is already deleted

**Events:**
- Emits `CommentUpdated`
...
- Preserves comment content for audit trail
- Prevents further replies to deleted comments
...
Comments are stored as shared objects, enabling efficient off-chain querying:

```typescript
// Query all comments for an article
const comments = await client.queryEvents({
  query: {
    MoveEventType: `${PACKAGE_ID}::comment::CommentCreated`,
  },
  filter: {
...
// Build comment tree from events
