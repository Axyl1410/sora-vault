...
Emitted when a subscription tier is upgraded or downgraded.
...
| Field | Type | Description |
|-------|------|-------------|
| `old_subscription_id` | `ID` | ID of the old subscription (destroyed) |
| `new_subscription_id` | `ID` | ID of the new subscription |
| `publication_id` | `ID` | Publication being subscribed to |
| `subscriber` | `address` | Subscriber's address |
| `old_tier` | `u8` | Previous tier (0=Free, 1=Basic, 2=Premium) |
| `new_tier` | `u8` | New tier (0=Free, 1=Basic, 2=Premium) |
| `new_expires_at` | `u64` | New expiration timestamp (seconds) |

**When Emitted:**
- `subscription::change_tier()` succeeds
- `subscription::change_tier_with_kiosk()` succeeds

**Use Cases:**
- Track tier changes
- Update subscription caches
- Analytics for upgrade/downgrade patterns
- Notify subscribers of tier changes
- Revenue tracking (upgrade payments)

**Example Event Data:**
```json
{
  "type": "0xabc::subscription::SubscriptionTierChanged",
  "old_subscription_id": "0xabc...",
  "new_subscription_id": "0xdef...",
  "publication_id": "0x123...",
  "subscriber": "0x456...",
  "old_tier": 1,
  "new_tier": 2,
  "new_expires_at": "1738368000"
}
```
...
Emitted when a new comment is created on an article.
...
| `article_id` | `ID` | Article being commented on |
| `author` | `address` | Comment author's address |
| `parent_comment_id` | `Option<ID>` | Parent comment ID if this is a reply (None for top-level) |
| `content_hash` | `vector<u8>` | Hash of comment content (for event indexing) |
| `created_at` | `u64` | Creation timestamp (seconds) |

**When Emitted:**
- `comment::create_comment()` succeeds
- `comment::reply_to_comment()` succeeds

**Use Cases:**
- Build comment threads
- Notify article authors
...
```json
{
  "type": "0xabc::comment::CommentCreated",
  "comment_id": "0x789...",
  "article_id": "0x456...",
  "author": "0x123...",
...
```json
{
  ...
  "article_id": "0x456...",
  "author": "0x999...",
  "parent_comment_id": "0x789...",
  "content_hash": "0xdef456...",
  "created_at": "1703894500"
}
```

### `CommentUpdated`

Emitted when a comment is updated by its author.
...
| Field | Type | Description |
|-------|------|-------------|
| `comment_id` | `ID` | Comment being updated |
| `author` | `address` | Comment author (for verification) |
| `new_content_hash` | `vector<u8>` | Hash of new content |
| `updated_at` | `u64` | Update timestamp (seconds) |

**When Emitted:**
- `comment::update_comment()` succeeds

**Use Cases:**
- Track comment edits
- Update cached comment content
- Audit trail for content changes
- Display "edited" indicators

**Example Event Data:**
```json
{
  "type": "0xabc::comment::CommentUpdated",
  "comment_id": "0x789...",
  "author": "0x123...",
  "new_content_hash": "0xnew123...",
  "updated_at": "1703894600"
}
```

### `CommentDeleted`

Emitted when a comment is soft-deleted by its author.
...
| Field | Type | Description |
|-------|------|-------------|
| `comment_id` | `ID` | Comment being deleted |
| `author` | `address` | Comment author (for verification) |
| `deleted_at` | `u64` | Deletion timestamp (seconds) |

**When Emitted:**
- `comment::delete_comment()` succeeds

**Use Cases:**
- Hide deleted comments from UI
...
```json
{
  "type": "0xabc::comment::CommentDeleted",
  "comment_id": "0x789...",
  "author": "0x123...",
  "deleted_at": "1703894700"
}
```
