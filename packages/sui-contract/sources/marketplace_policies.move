...
module private_publishing::marketplace_policies {
    ...
    // === TransferPolicy Creation ===

    /// Creates a new TransferPolicy for SubscriptionNFT
    /// Requires Publisher to prove package ownership
    /// The policy is shared and the cap is transferred to the caller
    /// After creation, call add_royalty_rule to configure royalty
    #[allow(lint(share_owned))]
    public fun create_transfer_policy(
        publisher: &Publisher,
        ctx: &mut TxContext
    ) {
        let (policy, cap) = policy::new<SubscriptionNFT>(publisher, ctx);
        
        // Share the policy so it can be used by anyone
        // Use public_share_object because TransferPolicy is from another module
        transfer::public_share_object(policy);
        
        // Transfer the cap to the caller so they can manage the policy
        // Use public_transfer because TransferPolicyCap is from another module
        transfer::public_transfer(cap, ctx.sender());
    }

    /// Creates TransferPolicy and adds default royalty rule in one transaction
    /// This is a convenience function for initial setup
    #[allow(lint(share_owned))]
    public fun create_transfer_policy_with_royalty(
        publisher: &Publisher,
        royalty_bps: u16,
        min_royalty: u64,
        ctx: &mut TxContext
    ) {
        let (mut policy, cap) = policy::new<SubscriptionNFT>(publisher, ctx);
        
        // Add royalty rule
        add_royalty_rule(&mut policy, &cap, royalty_bps, min_royalty);
    ...
    #[test_only]
    /// Creates a Publisher for testing (can be used in test modules)
    public fun create_test_publisher(ctx: &mut TxContext): Publisher {
        package::test_claim(MARKETPLACE_POLICIES {}, ctx)
    }
