...
module private_publishing::article {
    ...
    public struct ArticleEpochExtended has copy, drop {
        article_id: ID,
        extender: address,
    ...
    public fun publish_article(
        ...
        tier: Tier,
        published_at: u64,
        image_metadata: String,             // JSON metadata for images
        deposit: Coin<SUI>,
        ctx: &mut TxContext
    ) {
        // Verify ownership
    ...
    /// Updates article metadata (title, excerpt, and images)
    ...
    /// Extends the storage epochs for an article
    /// Anyone can call this function to extend storage duration
    /// This is useful for community-supported content preservation
    /// Note: Actual Walrus blob extension happens off-chain via frontend
    public entry fun extend_article_epochs(
        article: &Article,
        additional_epochs: u64,
        clock: &Clock,
        ctx: &TxContext
    ) {
        event::emit(ArticleEpochExtended {
            article_id: object::id(article),
            extender: ctx.sender(),
            additional_epochs,
